...
const Header = () => {
    ...
    return (
        <div className='flex gap-3 md:gap-2 items-center p-6'>
            ...
            <button className='p-2 rounded-full px-4 font-semibold' onClick={() => router.push('./pin-builder')}>Create</button>
            <div className='bg-[#e9e9e9] p-3  gap-3 rounded-full items-center w-full flex'>
                <MagnifyingGlassIcon className='h-5' />
                <input type="text" placeholder='search' className='outline-none w-full cursor-text bg-transparent' />
            </div>
            {/* <BellIcon className='h-3 hover:bg-gray-300 p-2 rounded-full cursor-pointer ' />
            <ChatBubbleBottomCenterIcon className='h-3 hover:bg-gray-300 p-1 rounded-full cursor-pointer' /> */}
            {session?.user ? <Image src={session?.user?.image}
                onClick={() => router.push('/'+session.user.email)}
                alt='Profile' height={50} width={50}
                className='hover:bg-gray-300 p-2 rounded-full cursor-pointer' /> :
